...
var Animalese = function (letters_file, onload) {
  this.Animalese = function (script, shorten = false, pitch = 1.0) {
    ...
    for (
      var c_index = 0;
      c_index < processed_script.length;
      c_index++
    ) {
      ...
      if (c >= "A" && c <= "Z") {
        ...
        for (var i = 0; i < output_samples_per_letter; i++) {
          data[
            c_index * output_samples_per_letter + i
          ] = this.letter_library[
...
window.animaleseIsReady = false;
window.animaleseBuffer = [];

window.sayAnimalese = (text) => {
  if (window.animaleseIsReady) {
    sayText(text);
    return;
  }

  // otherwise add to buffer
  animaleseBuffer.push(text);
};

var synth = new Animalese("lib/animalese.wav", function () {
  window.animaleseIsReady = true;
  animaleseBuffer.forEach(sayText);
});
